...
export default function MultiActionAreaCard({ text, cosmetic, navigationType, style, menStyle }) {
  ...
  const updatedProductsWomen = products.filter(product => product.name.includes('omen'));
  const updatedProductsMen = products.filter(product => !product.name.includes('omen'));

  const productsToDisplay = isWomenPage ? updatedProductsWomen : isMenPage ? updatedProductsMen : products;
  ...
  return (
    ...
    >
      ...
      <Box
        sx={{
          display: 'grid',
          gridTemplateColumns: isWomenPage ? 'repeat(3, 1fr)' : 'repeat(4, 1fr)',
          gap: '30px',
